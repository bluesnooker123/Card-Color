...
var selected_reject_color = {};
var selected_reject_color_id = {};
var reject_color_count = 0;
...
$(document).ready(function(){
  ...
  $('#btn_next').click(function(){
    if(reject_color_count < 2){
        swal("Please select a color for every card");
    }
    else{
      // console.log(JSON.stringify(selected_reject_color));
      sessionStorage.setItem("selected_reject_color", JSON.stringify(selected_reject_color));
      sessionStorage.setItem("selected_reject_color_id", JSON.stringify(selected_reject_color_id));
      location.href="/step_6.php";
    }
  });
  ...
  $('.color-img').click(function(event){
    ...
    if(selected_reject_color[selected_card.id] == ''){
      reject_color_count++;
    }
    selected_reject_color[selected_card.id] = temp;
    var temp_id = selected_color.id;
    selected_reject_color_id[selected_card.id] = temp_id;

    console.log(selected_reject_color);
    console.log(selected_reject_color_id);
...
function init_array(){
  var reject_card_id = sessionStorage.getItem('selected_reject_cards').split(',');
  for(i=0; i < sessionStorage.getItem('reject_count'); i ++){
    selected_reject_color[reject_card_id[i]] = '';
    selected_reject_color_id[reject_card_id[i]] = '';
  }
  console.log(selected_reject_color);
}
